...
void timeDown() { //Decrement time variable, counting in seconds
  ...
  if (timer < 0) {
    ...
    tone(50,random(100,100)); //out pin 46, random freq, for 300 ms
    
    while(digitalRead(29) == HIGH){
      //Do nothing, wait for switch to be reset
    }
    loop(); //reset loop()
    //THIS METHOD OF RESETTING IS FLAWED, THE PROBLEM WILL CRASH EVENTUALLY BY DOING THIS.
...
//Functions are given in reverse order here so that they can be nested inside each other by being declared within the correct scope.
...
int puzzle_3_1() {
  ...
  Serial.println("WELCOME TO PUZZLE 3: THE ONE WHERE YOU UNDO THE WIRES!!!!!!!!!!!!!!1");
  int solved = 0;
  delay(1000);
  while (solved == 0) {
    timeDisp.check(); //Refresh digit display every millisecond
...
int puzzle_2_1() {
  ...
  while (solved == 0) {
    timeDisp.check(); //Refresh digit display every millisecond
    timeIterate.check(); //start counting down time variable.  Will decrement every second.
    if (digitalRead(A13) == HIGH && val == 0) { //first press = blue button
      timeDisp.check(); //Refresh digit display every millisecond
      timeIterate.check(); //start counting down time variable.  Will decrement every second.
    ...
    else if (digitalRead(A12) == HIGH && val == 1) { //Second press = white button
      timeDisp.check(); //Refresh digit display every millisecond
      timeIterate.check(); //start counting down time variable.  Will decrement every second.
    ...
    else if (digitalRead(A15 ) == HIGH && val == 2) { //third press = red button
      timeDisp.check(); //Refresh digit display every millisecond
      timeIterate.check(); //start counting down time variable.  Will decrement every second.
      Serial.println("Blue!");
      pushCounter++;
    ...
    else if (digitalRead(A14) == HIGH && val == 3) { //fourth press = green button
      timeDisp.check(); //Refresh digit display every millisecond
      timeIterate.check(); //start counting down time variable.  Will decrement every second.
      Serial.println("Blue!");
      pushCounter++;
  ...
  while (solved == 0) {
    ...
    while (digitalRead(A9) == HIGH && solved == 0) {
      ...
      while (digitalRead(A10) == HIGH && solved == 0) {
        timeDisp.check(); //Refresh digit display every millisecond
        timeIterate.check(); //start counting down time variable.  Will decrement every second.
        Serial.print("Number 3 is in place");
        while (digitalRead(A8) == LOW && solved == 0) {
          timeDisp.check(); //Refresh digit display every millisecond
          timeIterate.check(); //start counting down time variable.  Will decrement every second.
        }
        while (digitalRead(A8) == HIGH && solved == 0) {
          timeDisp.check(); //Refresh digit display every millisecond
          timeIterate.check(); //start counting down time variable.  Will decrement every second.
          Serial.println("number 1 is in place");
          while (digitalRead(A11) == LOW && solved == 0) {
            timeDisp.check(); //Refresh digit display every millisecond
            timeIterate.check(); //start counting down time variable.  Will decrement every second.
          }
          while (digitalRead(A11) == HIGH && solved == 0) {
            timeDisp.check(); //Refresh digit display every millisecond
            timeIterate.check(); //start counting down time variable.  Will decrement every second.
            Serial.println("4 is in place.Wires have been beat!");
            solved = 1;
          }
        }
      }
